function init(player, OPPONENT) {
  ...
  canvas.addEventListener("click", function (event) {
    ...
    if (board[i][getPosition.position].val) return;
    ...
    if (OPPONENT == "computer") {
      ...
      const randomElement =
        EMPTY_SPACES[Math.floor(Math.random() * EMPTY_SPACES.length)];

      let space = getIJ(randomElement.id);
      board[space.i][space.j].val = player.computer;
      gameData[board[space.i][space.j].id] = player.computer;
      setTimeout(() => {
        drawOnBoard(player.computer, space.i, space.j);
      }, 1000);
      // Check if the play wins
  ...
  function getId(val) {
    ...
    //   console.log(board[val]);
    //   for (let j = 0; j < board[val].length; j++) {
    //     console.log(board[val][j]);
    //   }
    //   // if (!board[val][selectedArray[i]].val) {
    ...
    // };
    let selectableArray = board[val].filter((x) => !x.val);
    console.log(selectableArray);
    console.log(selectableArray[selectableArray.length - 1]);

    return {
      id: selectableArray[selectableArray.length - 1].val,
      position: selectableArray.length - 1,
    };
